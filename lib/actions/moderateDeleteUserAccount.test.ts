...
describe("moderateDeleteUserAccount", () => {
  it("returns unauthorized and never attempts deletion when not an admin", async () => {
    ...
    const result = await moderateDeleteUserAccount("target-1", "reason", "other");
  ...
  it("rejects self-targeting", async () => {
    const result = await moderateDeleteUserAccount("admin-1", "reason", "other");
  ...
  it("rejects an invalid reason category", async () => {
    ...
    const result = await moderateDeleteUserAccount("target-1", "reason", "not_a_category");
  ...
  it("rejects a missing reason", async () => {
    const result = await moderateDeleteUserAccount("target-1", "   ", "other");
