...
const { unfollowRoute } = await import("./unfollowRoute");
...
describe("unfollowRoute", () => {
  it("returns an error when unauthenticated", async () => {
    currentUser = null;

    const result = await unfollowRoute("bank-a", "bank-b");

    expect(result).toEqual({ error: "You must be signed in." });
    expect(deleteMock).not.toHaveBeenCalled();
  });

  it("deletes the follow row scoped to the caller's own user_id and the exact pair", async () => {
    const result = await unfollowRoute("bank-a", "bank-b");

    expect(result).toEqual({ success: true });
    expect(fromMock).toHaveBeenCalledWith("watchlist_route_follows");
    expect(eqMock).toHaveBeenCalledWith("user_id", "user-1");
    expect(eqMock).toHaveBeenCalledWith("from_bank_id", "bank-a");
    expect(eqMock).toHaveBeenCalledWith("to_bank_id", "bank-b");
  });
  ...
  it("surfaces a delete error as a failure", async () => {
    ...
    expect(result).toEqual({ error: "Failed to unwatch this route." });
  });
});
