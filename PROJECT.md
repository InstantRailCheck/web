...
## Version 8.14.6 (v8.14.6 — shipped July 21 2026)

**Code-review follow-ups on the v8.14.2-v8.14.5 duplicate-institution work**
- **Duplicate confirmation was too permissive**: `duplicateInstitutions.mjs`'s corroboration check only tested for *contradiction* (mismatched address/assets), never required *corroboration* — two banks sharing a name with both sides' address/assets blank were confirmed on "nothing disagrees," not "something agrees," across both matching passes. Now requires at least one genuinely matching address, website, or `total_assets` before confirming; anything short of that is flagged for manual review instead (`website` added to both scripts' fetch columns for this)
- **Alias-inflated `name_normalized` broke exact-identity lookups, not just fuzzy grouping** — found this was live and active, not just in already-fixed audit scripts: `lib/actions/addBank.ts`'s own duplicate-name check and `lib/railParticipation.ts`'s live sibling-group query both compared a bare-name normalization against the `aka_names`-inflated `name_normalized` column, so adding a bank with the same name as an alias-carrying institution (e.g. "Bank of America, National Association") would not have been recognized as a duplicate — silently recreating the exact bug class this whole effort has been cleaning up. New generated column `banks.name_only_normalized` (bare name, never aliases) fixes both. `lib/railParticipation.ts` also gained the `is_active` filter already applied elsewhere
- **Discovered while fixing the above**: `backfill-rail-participation.mjs` and `audit-duplicate-name-rail-flags.mjs` (both already touched in v8.14.4/8.14.5) still preferred the stored, alias-inflated `name_normalized` over a bare-name computation when grouping duplicate-name siblings — silently splitting one real duplicate-name group into two whenever only one sibling had aliases attached. Fixed consistently across all three rail scripts (`resolve-ambiguous-true-rail-flags.mjs` too), which surfaced a much larger backlog than expected: flagged rail-ambiguity groups went from 2 to 32 once real siblings were correctly reunited. Re-ran the full pipeline against production: `backfill-rail-participation.mjs` (0 changes needed — `resolveRailFlag` never downgrades an already-true value by design), then `resolve-ambiguous-true-rail-flags.mjs --apply` (68/68 field corrections applied, all `true → null`, never `true → false`), confirmed both audits exit clean (0 confirmed, 14 already-known institution-name collisions; 0 rail-flag ambiguities) afterward
- **Duplicate merging wasn't atomic**: `apply-duplicate-merge.mjs` performed the old-row deactivation and the surviving row's `aka_names` update as two independent requests. New `merge_duplicate_bank` RPC (modeled directly on `apply_bank_correction`'s transactional shape) does both in one transaction
- **Audit baselines were too coarse**: keying only on a bank's own slug (or `slug:rail`) let a baseline entry keep suppressing a group even after its actual candidate/member composition changed. Keys now include the full sorted candidate/member-slug set; both baseline files reseeded under the new format
...
## Version 10.0.0 (v10.0 — shipped August 6 2026)

**U.S. Instant Payments Coverage Report (`/research/instant-payments`)** — a public, citable data page aggregating FedNow/RTP/Zelle participation across the whole active directory, aimed at journalists/researchers rather than someone looking up one bank. Built from a ChatGPT-drafted pitch, reviewed against this project's Data Principles and revised before implementation: institution-directory freshness and rail-participation freshness are tracked and labeled separately, since they come from genuinely different pipelines (`finalize_sync_run` never touches the rail-participation flags — that's `scripts/backfill-rail-participation.mjs`, a separate, currently-unlogged step). The page cites "rail participant source lists last downloaded," not "coverage last verified," because the backfill step's own completion time isn't knowable yet — see the deferred `rail_participation_sync_log` note below
- New `lib/coverageReport.ts`: pure `computeCoverageReport()` over already-fetched bank rows — `false`/`null` are never merged for any rail, `total_assets`/`state` nulls bucket into explicit "Unknown"/"Not on file" rows rather than being dropped from denominators, asset tiers and percentages are exactly boundary-tested
- New `lib/coverageReportFreshness.ts`: the DB-touching layer, using `createAdminClient()` (required — `sync_runs` has RLS enabled with only a `service_role` grant, so the regular client would silently return zero rows on this table rather than erroring) and `sync_runs.finished_at` (not `started_at` — the latter is fine for `/api/health`'s operational monitoring, not for a public "as of" claim). Wrapped in `unstable_cache` with a log-then-rethrow wrapper, same pattern as `lib/needsFreshReports.ts` — a failed background revalidation can't poison the cache with an error, and the expensive ~9-page `fetchAllBanks()` aggregation runs at most once per 4h no matter how many visitors hit the page in between
- New `components/CoverageBarChart.tsx`: plain CSS three-segment bar (confirmed/not confirmed/unknown), no charting dependency — every count is also rendered as text, not color-only
- New `buildDatasetJsonLd()` in `lib/jsonLd.ts` — no `distribution` (no CSV export yet) or `license` (undecided) field, matching the same "don't claim what isn't true" standard as everything else here
- Extracted the previously-duplicated `ZELLE_INCOMPLETE_CAVEAT` string (copy-pasted between `lib/bankProfile.ts` and `app/rails/page.tsx`) into one export in `lib/railDisplayName.ts`
- **Deferred to a follow-up**: a real `rail_participation_sync_log` table (written only after participant downloads *and* the backfill both succeed) — the actual fix that would let the rail-coverage numbers be labeled "verified" instead of "downloaded." Also deferred: CSV export (`lib/csv.ts`'s `toCsv()` already exists for this), a "copy citation" block, `Article` JSON-LD, and a press-summary pull-quote

## Version 10.1.0 (v10.1 — shipped August 7 2026)

**Real rail-participation freshness (`rail_participation_sync_log`)** — the v10.0 follow-up: `scripts/backfill-rail-participation.mjs` now logs its own completion, so `/research/instant-payments` can finally say "rail participation last verified" instead of the v10.0 stopgap ("source lists last downloaded"), which could never confirm the bank flags were actually backfilled from what was downloaded
- New `rail_participation_sync_log` table, written only when the backfill script completes a run with **zero per-bank update failures** — a partial-failure run must never look like a clean one. Fixed a real, independent bug in the same script while adding this: it previously exited 0 unconditionally, even if every single bank's update failed, since the per-bank error was only ever logged, never tracked or checked before exit
- Same migration also grants `service_role` explicit privileges on `fednow_participants`/`rtp_participants`/`zelle_participants` — discovered these never got the same fix `20260714030000_make_service_role_grants_replayable.sql` gave four other tables; production only worked on legacy dashboard-inherited privileges that predate RLS, and a fresh migration replay (this release's own `scripts/db-tests/railParticipationSyncLog.check.mjs` was the first thing to ever try) failed at the SQL privilege layer
...
## Version 10.4.0 (v10.4 — shipped August 9 2026)

**`bank_asset_backfill_log` — the last coverage-report freshness gap** — code review findings following v10.3.2: `scripts/backfill-bank-assets.mjs` (writes `banks.total_assets`, which drives `/research/instant-payments`'s `byAssetTier` breakdown) was the only data-mutating script in the project with no log of its own, so the coverage report's `dateModified` could go stale silently whenever asset data changed. New `bank_asset_backfill_log` table, same shape and same "only written on a zero-failure run" contract as `rail_participation_sync_log` (v10.1) — also fixed a latent bug while adding it: the script's per-bank `.update()` errors were previously silently dropped, never tracked or surfaced. `CoverageFreshness` gains `assetDataAsOf`, folded into `dateModified` and shown as a fourth freshness line on the page; `null` until the first post-migration run of the monthly `sync-ncua-and-assets` job. This migration adds its `EXPECTED_RLS_ENABLED_TABLES`/`EXPECTED_POLICIES` manifest entries in the same commit, directly avoiding a repeat of the drift v10.3.2 just fixed
- Also fixed: `backfill-rail-participation.mjs`'s `banks_processed` count included inactive banks the loop actually skips — now counts only active banks actually considered
...
## Version 11.0.0 (v11.0 Phase 1 — shipped August 10 2026)

**Watchlists — the foundational retention loop** — the site handled discovery, evidence, research, and contributions well, but there was no reason to come back after checking a bank once. v11.0 starts the loop (`Search → verify → follow → receive an update → return → contribute`) with just the follow relationship and privacy model — no activity feed yet, that's Phase 2, once this foundation is right
- New `watchlist_bank_follows`/`watchlist_route_follows` tables — directional pair for routes (matching `route_reports`/`route_requests`), `on delete cascade` on both `user_id` and `bank_id` (a watchlist is purely personal with no communal evidence value once its owner is gone, same treatment as `webhooks` — unlike `route_reports`/`edd_reports`, which anonymize instead). Zero RLS policies, admin-client-only reads/writes gated by a Server Action's `getUser()` check — matches the dominant convention (`route_requests`) rather than the `auth.uid()`-scoped SELECT policy a 2026-07-10 migration comment had anticipated; deliberately chosen to keep the "every private table has zero client policies" invariant `scripts/rlsManifest.mjs` enforces fully intact. Both tables' manifest entries land in the same migration/commit — learned from the `route_requests`/`bank_asset_backfill_log` drift found and fixed the week before
- New `components/FollowBankButton.tsx` (bank profile page) and `components/WatchRouteButton.tsx` (after a real route check in `RouteSearch.tsx`) — both directly modeled on `RequestRouteButton.tsx`'s auth-tracking + Server Action shape, plus a second effect (mirroring `PasskeyManager.tsx`'s fetch-after-auth pattern) to learn current follow state on mount, since unlike a route request this is a toggle
- New `components/WatchlistDashboard.tsx` under `/account`, directly modeled on `PasskeyManager.tsx`'s structure — `app/account/page.tsx` itself gains no server-side auth check, matching its established philosophy of every section self-gating client-side
- A followed bank that's gone inactive still displays truthfully in the dashboard rather than silently vanishing or claiming to still be active — "unknown is better than wrong" applies to a user's own watchlist too. Bank-merge propagation is an explicit non-goal for this phase
...
- **New indexes on tables that take writes in normal operation** (`route_reports`, `moderation_actions`, etc.) should use `create index concurrently if not exists` rather than a plain `create index` — a plain index build holds a lock that blocks writes to the table for the build's duration. Confirmed empirically against this project's local Supabase stack that `CONCURRENTLY` runs fine through `supabase migration up`/`db push` (no implicit-transaction wrapping issue to work around). `20260726000000_add_route_reports_and_moderation_actions_indexes.sql` used the plain form and was safe in practice only because both tables were still near-empty (9 rows, 0 rows) at the time it ran — don't rely on that being true again
