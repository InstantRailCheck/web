...
- Fresh Supabase migration replay now explicitly restores the table/function grants production's dashboard-created tables already had, so the real database regression job reaches the behavior under test instead of failing at PostgreSQL's privilege layer. The atomic bank-attribution rollback check now requires the exact expected `23503` foreign-key error rather than treating any RPC error (including a permissions error) as proof of atomicity
